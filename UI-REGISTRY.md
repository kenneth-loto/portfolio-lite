...
_Baseline established via `/imprint audit` on 2026-07-09._
...
| Shadow           | none                                                      |
| Accent usage     | none                                                      |
| Ref support      | React 19 direct prop — accepts `ref` without `forwardRef` |

**Pattern notes:**
The `Section` component is the outer wrapper for a terminal output block. Three sub-components compose a single terminal "session": `SectionPrompt` (flex-col, gap-0.5) wraps `SectionPwd` + `SectionCommand`; `SectionTerminal` (flex-col, gap-4) wraps one or more prompt+command blocks. `SectionOutput` is a `grid auto-fill` for `ls`-style directory listings only; all other sections use a plain `<div>` for their output. `SectionPwd` now uses `break-all leading-relaxed` for better long-path wrapping. `SectionCommand` `$` prefix uses `text-muted-foreground`. `SectionOutput` uses `gap-y-2` (was `gap-y-0.5`).
...
| Border radius | none               |
| Text          | inherited          |
| Spacing       | `flex-col gap-0.5` |
| Hover state   | none               |
| Shadow        | none               |
| Accent usage  | none               |

**Pattern notes:**
Thin wrapper that groups `SectionPwd` + `SectionCommand` into a compact block. `gap-0.5` (2px) is the tightest gap used — intentionally minimal so the prompt line and command read as a single unit.
